...
class maTrackingEnv1(maTrackingBase):
    ...
    def __init__(self, hyp, q_true = 0.02, q = 0.01, is_training=True, known_noise=True):
        ...
        self.limit['state'] = [np.concatenate(([0.0, -np.pi, -2*self.vel_limit[0], -2*self.vel_limit[1], -50.0, 0.0]*self.num_targets, [0.0, -np.pi ])),
                               np.concatenate(([600.0, np.pi, 2*self.vel_limit[0], 2*self.vel_limit[1],  50.0, 2.0]*self.num_targets, [self.sensor_r, np.pi]))]
